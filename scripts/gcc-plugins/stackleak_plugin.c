...
static HOST_WIDE_INT track_frame_size = -1;
...
static unsigned int stackleak_tree_instrument_execute(void)
{
	...
	if (!prologue_instrumented) {
		...
		if (!single_pred_p(entry_bb)) {
//			gcc_assert(bb_loop_depth(entry_bb) || (entry_bb->flags & BB_IRREDUCIBLE_LOOP));
...
static bool stackleak_track_stack_gate(void)
{
	...
	if (lookup_attribute("no_instrument_function", DECL_ATTRIBUTES(current_function_decl)))
		return false;
