...
static void rap_hash_type_name(const_tree type, rap_hash_flags_t flags, unsigned char sip_hash[8])
{
	const_tree name;

	name = type_name(TYPE_MAIN_VARIANT(type));

	// handle typedefs of anonymous enums/structs/unions
	if (name == NULL_TREE) {
		tree t;

		// assume that the first variant of an otherwise anonymous type has the name we want
		// the first variant is the last in the chain of variants
		for (t = TYPE_MAIN_VARIANT(type); t; t = TYPE_NEXT_VARIANT(t)) {
			name = type_name(t);
		}
	}

	// in some situations the anonymous type isn't aliased directly at all (e.g., only a pointer to it is defined)
	// in such cases assume that its definition must be present in all TUs and thus its fields can be used in the type hash
	if (name) {
		...
		case ENUMERAL_TYPE:
			gcc_assert(TYPE_VALUES(type));
			for (field = TYPE_VALUES(type); field; field = TREE_CHAIN(field)) {
				const_tree purpose, value;
				HOST_WIDE_INT enumval;

				purpose = TREE_PURPOSE(field);
				gcc_assert(TREE_CODE(purpose) == IDENTIFIER_NODE);
				rap_fold_hash(sip_hash, (const unsigned char *)IDENTIFIER_POINTER(purpose), IDENTIFIER_LENGTH(purpose));

				value = TREE_VALUE(field);
				gcc_assert(TREE_CODE(value) == INTEGER_CST);
				enumval = tree_to_shwi(value);
				rap_fold_hash(sip_hash, (const unsigned char *)&enumval, sizeof enumval);
			}
		...
		case UNION_TYPE:
			...
			for (field = TYPE_FIELDS(type); field; field = TREE_CHAIN(field)) {
				...
				rap_hash_tree(TREE_TYPE(field), flags, sip_hash);
			}
...
// the first piece is a (hopefully) compiler independent encoding of the type, derived from the gcc tree code
...
static void rap_hash_tree(const_tree type, rap_hash_flags_t flags, unsigned char sip_hash[8])
{
	...
	case RECORD_TYPE:
		rap_hash_type_name(type, flags, sip_hash);
		break;
...
// erase the hash of deleted functions since their uid can be reused later
static void rap_hash_remove_node_data(cgraph_node_ptr node, void *data __unused)
{
	if (node->uid >= rap_cgraph_max_uid)
		return;

	if (rap_imprecise_hashes[node->uid].hash)
		rap_imprecise_hashes[node->uid].hash = 0;
}

// assume that all duplicated functions added after the frontend cannot be targets of indirect calls
static void rap_hash_duplicate_node_data(cgraph_node_ptr src __unused, cgraph_node_ptr dst, void *data __unused)
{
	if (dst->uid >= rap_cgraph_max_uid)
		return;

	rap_imprecise_hashes[dst->uid].hash = 0;
}

static void rap_hash_register_hooks(void)
{
	...
	node_removal_hook_holder = cgraph_add_node_removal_hook(&rap_hash_remove_node_data, NULL);
	node_duplication_hook_holder = cgraph_add_node_duplication_hook(&rap_hash_duplicate_node_data, NULL);
}
