/*
 * Copyright 2012-2018 by PaX Team <pageexec@freemail.hu>
...
//	e->probability = REG_BR_PROB_BASE;
	...
	e = make_edge(cond_bb, true_bb, EDGE_TRUE_VALUE | EDGE_PRESERVE);
	e->probability = REG_BR_PROB_VERY_UNLIKELY;
	...
	// insert call to builtin_trap or rap_ret_abort
	*gsi = gsi_start_bb(true_bb);
	...
	if (rap_ret_abort) {
		...
		// asm volatile(rap_ret_abort %0 : : "r"(new_retaddr));
...
#endif
		stmt = gimple_build_asm_vec(rap_ret_abort, inputs, NULL, NULL, NULL);
...
static tree get_call_target(rtx_insn *insn)
{
	rtx body;
	tree fn;
	...
	switch (GET_CODE(XEXP(body, 0))) {
	default:
		return NULL_TREE;

	case SYMBOL_REF:
		fn = SYMBOL_REF_DECL(XEXP(body, 0));
		gcc_assert(TREE_CODE(fn) == FUNCTION_DECL);
		return fn;

	case REG:
		fn = REG_EXPR(XEXP(body, 0));
		if (!fn)
			return fn;

		switch (TREE_CODE(fn)) {
		default:
			fprintf(stderr, "FPTR ");print_rtl_single(stderr, XEXP(body, 0));
			debug_tree(fn);
			gcc_unreachable();
...
static tree rap_find_retloc_mark(rtx_insn *insn)
{
	...
	for (hash = PREV_INSN(insn); hash && hash != PREV_INSN(BB_HEAD(bb)); hash = PREV_INSN(hash)) {
		tree computed_hash;
		rtx body;
...
static tree rap_get_direct_call_retloc_mark(rtx_insn *insn)
{
	...
	fntype = get_call_target(insn);
	if (fntype == NULL_TREE || TREE_CODE(fntype) != FUNCTION_DECL)
		return NULL_TREE;

	func_hash = rap_hash_function_decl(fntype, imprecise_rap_hash_flags);
...
#ifdef TARGET_386
	if (UNITS_PER_WORD == 8)
		mark = gen_rtx_ASM_OPERANDS(VOIDmode, ggc_strdup("jmp %l1 ; .byte 0x48, 0xb8 ; .quad %c0 ; .skip 8-(%l2-%l1),0xcc"), empty_string, 0, argvec, constraintvec, labelvec, INSN_LOCATION(insn));
	else
		mark = gen_rtx_ASM_OPERANDS(VOIDmode, ggc_strdup("jmp %l1 ; .byte 0xb8 ; .long %c0 ; .skip 6-(%l2-%l1),0xcc"), empty_string, 0, argvec, constraintvec, labelvec, INSN_LOCATION(insn));
#else
#error unsupported target
#endif
...
static unsigned int rap_mark_retloc_execute(void)
{
	...
	for (insn = get_insns(); insn; insn = NEXT_INSN(insn)) {
		...
		// skip indirect calls here that retpoline will handle later
		if (enable_type_nospec && rap_retpoline_gate() ) {
			rtx body;
