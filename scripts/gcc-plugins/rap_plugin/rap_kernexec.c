...
void (*kernexec_instrument_fptr)(gimple_stmt_iterator *);
void (*kernexec_instrument_retaddr)(rtx);
...
void kernexec_instrument_fptr_bts(gimple_stmt_iterator *gsi)
{
	...
	assign_intptr = gimple_build_assign(intptr, fold_convert_loc(loc, long_unsigned_type_node, old_fptr));
	gimple_set_location(assign_intptr, loc);
...
//	kernexec_mask = build_int_cstu(long_long_unsigned_type_node, 0xffffffff80000000ULL);
	orptr = fold_build2_loc(loc, BIT_IOR_EXPR, long_long_unsigned_type_node, intptr, kernexec_mask);
	...
	assign_new_fptr = gimple_build_assign(new_fptr, fold_convert_loc(loc, TREE_TYPE(old_fptr), intptr));
	gimple_set_location(assign_new_fptr, loc);
...
static bool kernexec_cmodel_check(void)
{
	...
	// vsyscall code runs in userland
	if (!strncmp(TREE_STRING_POINTER(section), ".vsyscall_", 10))
		return false;

	// early startup code runs at physical addresses, not virtual
	if (!strcmp(TREE_STRING_POINTER(section), ".head.text.startup"))
		return false;
	if (!strcmp(TREE_STRING_POINTER(section), ".init.text.startup"))
		return false;
