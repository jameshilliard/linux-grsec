...
tree get_rap_hash(gimple_seq *stmts, location_t loc, tree fptr, HOST_WIDE_INT rap_hash_offset)
{
	...
	fptr2 = make_ssa_name(fptr2, NULL);
	assign_hash = gimple_build_assign(fptr2, fold_convert_loc(loc, build_pointer_type(TREE_TYPE(target_hash)), fptr));
	gimple_set_location(assign_hash, loc);
	SSA_NAME_DEF_STMT(fptr2) = assign_hash;
	gimple_seq_add_stmt(stmts, assign_hash);
	fptr = gimple_get_lhs(assign_hash);
...
static void rap_stash_retloc(gimple_stmt_iterator *gsi, tree computed_hash)
{
	gimple stmt;
...
//	e->probability = REG_BR_PROB_BASE;
	...
	*gsi = gsi_start_bb(true_bb);
	...
	if (rap_call_abort) {
		...
		// asm volatile("rap_call_abort %0" : : "r"(fptr));
...
#endif
		stmt = gimple_build_asm_vec(rap_call_abort, inputs, NULL, NULL, NULL);
		gimple_asm_set_volatile(as_a_gasm(stmt), true);
	...
	FOR_EACH_BB_FN(bb, cfun) {
		...
		for (gsi = gsi_start_bb(bb); !gsi_end_p(gsi); gsi_next(&gsi)) {
			...
			case ADDR_EXPR:
				...
				rap_stash_retloc(&gsi, computed_hash);
				continue;
