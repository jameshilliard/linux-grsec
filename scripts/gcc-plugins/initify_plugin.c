...
static struct attribute_spec nocapture_attr = ATTRIBUTE_SPEC_INIT("nocapture", 0, -1, true, false, false, handle_nocapture_attribute, false, NULL);
static struct attribute_spec unverified_nocapture_attr = ATTRIBUTE_SPEC_INIT("unverified_nocapture", 0, -1, true, false, false, handle_unverified_nocapture_attribute, false, NULL);
...
static bool search_init_exit_callers(void)
{
	...
	FOR_EACH_FUNCTION(node) {
		...
		for (e = node->callees; e; e = e->next_callee) {
			...
			// Can't change the section of aliases directly
			if (e->callee->alias)
				continue;
