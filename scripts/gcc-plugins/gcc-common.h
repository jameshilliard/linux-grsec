...
#ifdef _WIN32
#define __visible __declspec(dllexport)
#else
...
static inline void add_type_attr(tree type, const char *attr)
{
	tree main_variant = TYPE_MAIN_VARIANT(type);

	TYPE_ATTRIBUTES(main_variant) = copy_list(TYPE_ATTRIBUTES(main_variant));
	TYPE_ATTRIBUTES(main_variant) = tree_cons(get_identifier(attr), NULL_TREE, TYPE_ATTRIBUTES(main_variant));
...
#if BUILDING_GCC_VERSION < 4007
#define ATTRIBUTE_SPEC_INIT(NAME, MIN, MAX, DECL, TYPE, FUNC, HANDLER, IDENTITY, EXCLUDE) \
{							\
	.name				= NAME,		\
	.min_length			= MIN,		\
	.max_length			= MAX,		\
...
#elif BUILDING_GCC_VERSION >= 4007 && BUILDING_GCC_VERSION < 8000
#define ATTRIBUTE_SPEC_INIT(NAME, MIN, MAX, DECL, TYPE, FUNC, HANDLER, IDENTITY, EXCLUDE) \
{ \
	.name				= NAME,		\
	.min_length			= MIN,		\
	...
	.decl_required			= DECL,		\
	.type_required			= TYPE,		\
	.function_type_required		= FUNC,		\
	.handler			= HANDLER,	\
	.affects_type_identity		= IDENTITY,	\
}
#else
#define ATTRIBUTE_SPEC_INIT(NAME, MIN, MAX, DECL, TYPE, FUNC, HANDLER, IDENTITY, EXCLUDE) \
{ \
...
// isn't exactly equivalent unfortunately as it creates a varpool node if it doesn't exist yet
static inline varpool_node_ptr varpool_get_node(tree decl)
{
...
#define SET_INSN_LOCATION(INSN, LOC) INSN_LOCATOR(INSN) = (LOC)
#define vNULL NULL
...
static inline tree copy_ssa_name(tree var, gimple stmt)
{
	return make_ssa_name(SSA_NAME_VAR(var), stmt);
}

static inline struct loop *loop_outermost(struct loop *loop)
{
	return VEC_length(loop_p, loop->superloops) <= 1 ? loop : VEC_index(loop_p, loop->superloops, 1);
}
#endif

#if BUILDING_GCC_VERSION <= 4007 || (BUILDING_GCC_VERSION == 4008 && BUILDING_GCC_PATCHLEVEL < 5)
...
#define REG_BR_PROB_NEVER		0
#define REG_BR_PROB_VERY_UNLIKELY	(REG_BR_PROB_BASE / 2000 - 1)
#define REG_BR_PROB_UNLIKELY		(REG_BR_PROB_BASE / 5 - 1)
#define REG_BR_PROB_EVEN		(REG_BR_PROB_BASE / 2)
#define REG_BR_PROB_LIKELY		(REG_BR_PROB_BASE - PROB_UNLIKELY)
#define REG_BR_PROB_VERY_LIKELY		(REG_BR_PROB_BASE - PROB_VERY_UNLIKELY)
#define REG_BR_PROB_ALWAYS		(REG_BR_PROB_BASE)
...
#define SET_INSN_LOCATION(INSN, LOC) INSN_LOCATION(INSN) = (LOC)
#endif

#if (BUILDING_GCC_VERSION == 4008 && BUILDING_GCC_PATCHLEVEL == 5) || BUILDING_GCC_VERSION == 4009
enum expand_modifier {
	EXPAND_NORMAL = 0,
	EXPAND_STACK_PARM,
	EXPAND_SUM,
...
{
	if (value)
		DECL_SECTION_NAME(node) = build_const_char_string(strlen(value) + 1, value);
	else
		DECL_SECTION_NAME(node) = NULL;
}

#ifdef __cplusplus
static inline bool gimple_assign_cast_p(const_gimple s)
{
	return gimple_assign_cast_p(CONST_CAST_GIMPLE(s));
}
#endif

typedef unsigned short poly_uint16_pod;
typedef HOST_WIDE_INT poly_int64_pod;
typedef unsigned HOST_WIDE_INT poly_uint64_pod;
//typedef offset_int poly_offset_int_pod;
//typedef wide_int poly_wide_int_pod;
//typedef widest_int poly_widest_int_pod;

typedef unsigned short poly_uint16;
typedef HOST_WIDE_INT poly_int64;
typedef unsigned HOST_WIDE_INT poly_uint64;
//typedef offset_int poly_offset_int;
//typedef wide_int poly_wide_int;
//typedef wide_int_ref poly_wide_int_ref;
//typedef widest_int poly_widest_int;

typedef double_int wide_int;
#endif
...
template <>
template <>
inline bool is_a_helper<const gcond *>::test(const_gimple gs)
{
...
#define get_inner_reference(exp, pbitsize, pbitpos, poffset, pmode, punsignedp, preversep, pvolatilep, keep_aligning)	\
	({ (void)(reversep); get_inner_reference(exp, pbitsize, pbitpos, poffset, pmode, punsignedp, pvolatilep, keep_aligning); })
#define gen_rtx_set(ARG0, ARG1) gen_rtx_SET(VOIDmode, (ARG0), (ARG1))

#ifndef LEGACY_INT_REG_P
#define LEGACY_INT_REG_P(X) (REG_P(X) && LEGACY_INT_REGNO_P(REGNO(X)))
#endif
#ifndef LEGACY_INT_REGNO_P
#define LEGACY_INT_REGNO_P(N) (IN_RANGE((N), AX_REG, SP_REG))
#endif

#ifndef FIRST_INT_REG
...
static inline void debug_bb(const_basic_block bb)
{
	debug_bb(CONST_CAST_BB(bb));
}
#else
#define debug_tree(t) debug_tree(CONST_CAST_TREE(t))
#define debug_gimple_stmt(s) debug_gimple_stmt(CONST_CAST_GIMPLE(s))
#define debug_bb(bb) debug_bb(CONST_CAST_BB(bb))
#endif
...
#if BUILDING_GCC_VERSION >= 4009 && BUILDING_GCC_VERSION < 8000
#define REG_BR_PROB_NEVER		0
#define REG_BR_PROB_VERY_UNLIKELY	PROB_VERY_UNLIKELY
#define REG_BR_PROB_UNLIKELY		PROB_UNLIKELY
#define REG_BR_PROB_EVEN		PROB_EVEN
#define REG_BR_PROB_LIKELY		PROB_LIKELY
#define REG_BR_PROB_VERY_LIKELY		PROB_VERY_LIKELY
#define REG_BR_PROB_ALWAYS		PROB_ALWAYS
#endif

#if BUILDING_GCC_VERSION >= 5000 && BUILDING_GCC_VERSION < 8000
#define cgraph_create_edge(caller, callee, call_stmt, count, freq, nest) \
...
#if BUILDING_GCC_VERSION < 8000
#ifdef __cplusplus
static inline void print_gimple_stmt(FILE *file, const_gimple g, int spc, int flags)
{
	print_gimple_stmt(file, CONST_CAST_GIMPLE(g), spc, flags);
}

static inline void print_gimple_expr(FILE *file, const_gimple g, int spc, int flags)
{
	print_gimple_expr(file, CONST_CAST_GIMPLE(g), spc, flags);
}
#else
#define print_gimple_stmt(file, g, spc, flags) print_gimple_stmt((file), CONST_CAST_GIMPLE(g), (spc), (flags))
#define print_gimple_expr(file, g, spc, flags) print_gimple_expr((file), CONST_CAST_GIMPLE(g), (spc), (flags))
#endif

#define E_BImode BImode
#define E_QImode QImode
#define E_HImode HImode
#define E_SImode SImode
#define E_DImode DImode
#define E_TImode TImode
#define E_OImode OImode
#if BUILDING_GCC_VERSION >= 4009
#define E_XImode XImode
#if BUILDING_GCC_VERSION >= 5000
#define E_BND32mode BND32mode
#define E_BND64mode BND64mode
#endif
#endif
#define E_QQmode QQmode
#define E_HQmode HQmode
#define E_SQmode SQmode
#define E_DQmode DQmode
#define E_TQmode TQmode
#define E_UQQmode UQQmode
#define E_UHQmode UHQmode
#define E_USQmode USQmode
#define E_UDQmode UDQmode
#define E_UTQmode UTQmode
#define E_HAmode HAmode
#define E_SAmode SAmode
#define E_DAmode DAmode
#define E_TAmode TAmode
#define E_UHAmode UHAmode
#define E_USAmode USAmode
#define E_UDAmode UDAmode
#define E_UTAmode UTAmode
#define E_SFmode SFmode
#define E_DFmode DFmode
#define E_XFmode XFmode
#define E_TFmode TFmode
#define E_SDmode SDmode
#define E_DDmode DDmode
#define E_TDmode TDmode
#endif

#if BUILDING_GCC_VERSION >= 8000
#define empty_string ""
#define REG_BR_PROB_NEVER		profile_probability::never()
#define REG_BR_PROB_VERY_UNLIKELY	profile_probability::very_unlikely()
#define REG_BR_PROB_UNLIKELY		profile_probability::unlikely()
#define REG_BR_PROB_EVEN		profile_probability::even()
#define REG_BR_PROB_LIKELY		profile_probability::likely()
#define REG_BR_PROB_VERY_LIKELY		profile_probability::very_likely()
#define REG_BR_PROB_ALWAYS		profile_probability::always()

#define cgraph_create_edge(caller, callee, call_stmt, count, freq, nest) \
	({ (void)(freq); (caller)->create_edge((callee), (call_stmt), (count)); })
#define cgraph_create_edge_including_clones(caller, callee, old_call_stmt, call_stmt, count, freq, nest, reason) \
	({ (void)(freq); ((caller)->create_edge_including_clones((callee), (old_call_stmt), (call_stmt), (count), (reason)); })

static inline void print_gimple_stmt(FILE *file, const_gimple g, int spc, dump_flags_t flags = TDF_NONE)
{
	print_gimple_stmt(file, CONST_CAST_GIMPLE(g), spc, flags);
}

static inline void print_gimple_expr(FILE *file, const_gimple g, int spc, dump_flags_t flags = TDF_NONE)
{
	print_gimple_expr(file, CONST_CAST_GIMPLE(g), spc, flags);
}

// hack to work around a mismatched prototype in gcc headers...
void debug_function(tree fn, dump_flags_t flags);
extern inline void debug_function(tree fn, int flags)
{
	debug_function(fn, (dump_flags_t)flags);
}
#endif
