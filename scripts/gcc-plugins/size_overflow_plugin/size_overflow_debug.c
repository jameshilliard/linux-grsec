...
void __unused print_interesting_stmts(interesting_stmts_t cur)
{
	print_next_interesting_function(cur->next_node);
	fprintf(stderr, "first_stmt:\n");
	debug_gimple_stmt(cur->first_stmt);
	fprintf(stderr, "num: %u\n", cur->num);
	fprintf(stderr, "orig_node:\n");
	debug_tree(cur->orig_node);
}
