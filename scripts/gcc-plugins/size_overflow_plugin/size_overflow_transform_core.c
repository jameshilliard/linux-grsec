/*
 * Copyright 2011-2018 by Emese Revfy <re.emese@gmail.com>
...
static gphi *overflow_create_phi_node(struct visited *visited, gphi *oldstmt, tree result)
{
	...
	// unfortunately gimple_phi_set_result in earlier gcc version don't do this...
...
//	e->probability = REG_BR_PROB_BASE;
	...
	e = make_edge(cond_bb, bb_true, EDGE_TRUE_VALUE);
	e->probability = REG_BR_PROB_VERY_UNLIKELY;

	e = make_edge(cond_bb, join_bb, EDGE_FALSE_VALUE);

	e =make_edge(bb_true, join_bb, EDGE_FALLTHRU);
	e->probability = REG_BR_PROB_ALWAYS;
...
static tree get_my_stmt_lhs(struct visited *visited, gimple stmt)
{
	...
	gcc_assert(gimple_code(stmt) != GIMPLE_NOP);
	gsi = gsi_for_stmt(stmt);
...
static bool handle_unsigned_neg_or_bit_not(interesting_stmts_t expand_from, const gassign *stmt)
{
	...
	if (!def_neg_stmt || gimple_code(def_neg_stmt) == GIMPLE_NOP)
		return false;
...
static tree handle_unary_rhs(interesting_stmts_t expand_from, gassign *stmt)
{
	...
	if (skip_types(rhs1)) {
		if (TREE_CODE(TREE_TYPE(rhs1)) != POINTER_TYPE && TREE_CODE(TREE_TYPE(rhs1)) != BOOLEAN_TYPE) {
			inform(gimple_location(stmt), "Skip this type\n");
			debug_tree(rhs1);
		}
		return create_assign(expand_from->visited, stmt, lhs, AFTER_STMT);
	}
...
{
	const_tree rhs_type;
	tree def_rhs1, lhs = gimple_assign_lhs(stmt);
	...
	rhs_type = TREE_TYPE(gimple_assign_rhs1(stmt));
	if (TYPE_MODE(rhs_type) == TImode && TYPE_UNSIGNED(rhs_type))
		return create_assign(expand_from->visited, def_stmt, lhs, AFTER_STMT);

	switch (TREE_CODE(def_rhs1)) {
	case SSA_NAME: {
