...
bool check_fields, check_fns, check_fnptrs, check_vars, search_missing_fns;
...
static struct attribute_spec size_overflow_attr = ATTRIBUTE_SPEC_INIT("size_overflow", 1, -1, true, false, false, handle_size_overflow_attribute, false, NULL);
static struct attribute_spec intentional_overflow_attr = ATTRIBUTE_SPEC_INIT("intentional_overflow", 1, -1, true, false, false, handle_intentional_overflow_attribute, false, NULL);
...
__visible int plugin_init(struct plugin_name_args *plugin_info, struct plugin_gcc_version *version)
{
	...
	for (i = 0; i < argc; ++i) {
		...
		if (!strcmp(argv[i].key, "search-missing-fns")) {
			search_missing_fns = true;
			continue;
		}
