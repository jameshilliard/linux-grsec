...
const struct size_overflow_hash *get_size_overflow_hash_entry(struct fn_raw_data *raw_data, bool insert_print)
{
...
const struct size_overflow_hash *get_size_overflow_hash_entry_tree(struct fn_raw_data *raw_data, bool hash_table)
{
	...
	if (hash_table == SIZE_OVERFLOW)
		return get_size_overflow_hash_entry(raw_data, INSERT_FN);
...
void print_missing_function(next_interesting_function_t node)
{
	...
	entry = get_size_overflow_hash_entry(&raw_data, PRINT_FN);
	if (entry)
		return;
