...
//#include <libgen.h> collides with musl's version
...
static void walk_use_def_next_functions_ternary(struct walk_use_def_data *use_def_data, const_tree lhs)
{
	...
	walk_use_def_next_functions(use_def_data, rhs3);
}
#endif
...
#if BUILDING_GCC_VERSION >= 4006
		case 4:
			walk_use_def_next_functions_ternary(use_def_data, lhs);
			return;
#endif
		default:
...
static bool already_in_the_hashtable(next_interesting_function_t next_node)
{
	...
	return get_size_overflow_hash_entry(&raw_data, INSERT_FN) != NULL;
}
...
static void walk_so_marked_fns(next_interesting_function_set *visited, next_interesting_function_t parent)
{
	...
	if (!search_missing_fns && !already_in_the_hashtable(parent))
		return;
