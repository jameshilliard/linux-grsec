...
static int elf_note_info_init(struct elf_note_info *info)
{
	...
	if (!info->prstatus)
		return 0;
	info->fpu = kzalloc(sizeof(*info->fpu), GFP_KERNEL);
	if (!info->fpu)
		return 0;
#ifdef ELF_CORE_COPY_XFPREGS
	info->xfpu = kzalloc(sizeof(*info->xfpu), GFP_KERNEL);
	if (!info->xfpu)
		return 0;
#endif
	return 1;
...
static int elf_core_dump(struct coredump_params *cprm)
{
	...
	if (segs - 1 > ULONG_MAX / sizeof(*vma_filesz))
		goto end_coredump;
	allocsize = (segs - 1) * sizeof(*vma_filesz);
	if (!allocsize)
		goto end_coredump;
	vma_filesz = vmalloc(allocsize);
	if (!vma_filesz)
		goto end_coredump;
...
static void elf_handle_mmap(struct file *file)
{
	...
	if ((file->f_op->mmap == NULL) || (file->f_op->open == NULL) ||
	    (file->f_path.mnt->mnt_flags & MNT_INTERNAL) || IS_PRIVATE(file_inode(file)))
		return;
