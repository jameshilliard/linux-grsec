...
static ssize_t mem_rw(struct file *file, char __user *buf,
			size_t count, unsigned long long *ppos, int write)
{
	...
	while (count > 0) {
		...
		this_len = access_remote_vm(mm, addr, page, this_len, write ? FOLL_WRITE : 0);
		if (!this_len) {
			if (!copied)
				copied = -EIO;
			break;
...
static __skip_size_overflow loff_t so_mem_lseek(struct file *file, loff_t offset, int orig)
{
	switch (orig) {
	case SEEK_SET:
		file->f_pos = offset;
