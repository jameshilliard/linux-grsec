...
{
	int ret;
	unsigned long __user *datap = (unsigned long __force_user *)data;
	...
	case PTRACE_GET_THREAD_AREA:
		...
		ret = do_get_thread_area(child, addr,
					(struct user_desc __force_user *)data);
		break;
	...
	case PTRACE_SET_THREAD_AREA:
		...
		ret = do_set_thread_area(child, addr,
					(struct user_desc __force_user *)data, 0);
		break;
#endif
				...
				struct siginfo *info)
{
	...
	info->si_addr = user_mode(regs) ? (void __force_user *)regs->ip : NULL;
}
