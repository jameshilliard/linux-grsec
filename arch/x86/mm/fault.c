...
#if defined(CONFIG_X86_64) && defined(CONFIG_PAX_MEMORY_UDEREF)
	if (boot_cpu_has(X86_FEATURE_UDEREF) && (boot_cpu_has(X86_FEATURE_PCID) || !boot_cpu_has(X86_FEATURE_SMAP))) {
		pgd = pgd_offset_cpu(smp_processor_id(), uaccess, address);
		if (pgd_none(*pgd)) {
			set_pgd(pgd, *pgd_ref);
...
static void dump_pagetable(pgd_t *base, unsigned long address)
{
	...
	printk("PGD %p %lx ", pgd, pgd_val(*pgd));
	...
	printk("PUD %p %lx ", pud, pud_val(*pud));
	...
	printk("PMD %p %lx ", pmd, pmd_val(*pmd));
	...
	printk("PTE %p %lx", pte, pte_val(*pte));
out:
	printk("\n");
	return;
bad:
		...
		unsigned long address)
{
	...
	dump_pagetable(__va(read_cr3() & PHYSICAL_PAGE_MASK), address);
}
