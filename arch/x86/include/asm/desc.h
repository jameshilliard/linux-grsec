...
static inline struct desc_struct *get_cpu_gdt_table_ro(unsigned int cpu)
{
#ifdef CONFIG_X86_64
...
{
	struct desc_ptr gdt_ro, gdt_rw;
	unsigned int cpu = raw_smp_processor_id();

	native_store_gdt(&gdt_ro);
	gdt_rw.address = (unsigned long)get_cpu_gdt_table(cpu);
	gdt_rw.size = GDT_SIZE - 1;
	load_gdt(&gdt_rw);
...
static inline void native_load_tls(struct thread_struct *t, unsigned int cpu)
{
	...
	for (i = 0; i < GDT_ENTRY_TLS_ENTRIES; i++)
		gdt[GDT_ENTRY_TLS_MIN + i + GDT_ENTRY_TLS_ENTRIES] = t->tls_array_inverted[i];
#endif
	pax_close_kernel();
}
