...
void gr_set_chroot_entries(struct task_struct *task, const struct path *path)
{
	...
	/* subtle:
	   set_ns -> install -> set_fs_root called with old task->nsproxy
	 */
...
gr_task_chroot_is_capable(const struct task_struct *task, const struct cred *cred, const int cap)
{
	...
	if (grsec_enable_chroot_caps && proc_is_chrooted(task)) {
		...
		rcu_read_lock();
		real_cred = __task_cred(task);
		rcu_read_unlock();

		/* ignore these added checks in the case of subjective credential elevation
		   e.g. in the case of overlayfs
		 */
		if (cred != real_cred)
			return 1;
