...
gr_check_protected_task_fowner(struct pid *pid, enum pid_type type)
{
	...
	if (unlikely(!(gr_status & GR_READY) || !pid))
        ...
        /* need to convert this type since it's illegal to use with do_each_pid_task,
           and we're only interested in processes */
	if (type == PIDTYPE_MAX)
		type = PIDTYPE_PID;
...
static int check_unsafe_flags(const struct acl_subject_label *acl, const struct dentry *dentry,
			      const struct vfsmount *mnt, const int unsafe_flags)
{
	/* if we have non-ptrace related unsafe flags, or only ptrace-related flags and we can't do a read-only ptrace
	   we do the GR_LEARN/GR_INHERITLEARN check after GR_PTRACERD so that GR_PTRACERD can be learned if needed, instead
	   of checking them in the caller and setting skip_check
	 */
	if (((unsafe_flags & ~(LSM_UNSAFE_PTRACE_CAP | LSM_UNSAFE_PTRACE)) || !gr_search_file(dentry, GR_PTRACERD, mnt)) &&
	    !(acl->mode & (GR_LEARN | GR_INHERITLEARN))) {
		  ...
		  const int unsafe_flags)
{
	...
	*/
	...
	if (!skip_check && check_unsafe_flags(task->acl, dentry, mnt, unsafe_flags))
		return -EACCES;


	if (inherited && (retmode & GR_INHERIT) && (retmode & GR_AUDIT_INHERIT))
...
void gr_save_rbac_exec_state(struct gr_rbac_exec_state *state)
{
	...
	state->acl = task->acl;
	state->exec_file = task->exec_file;
	state->is_writable = task->is_writable;
	state->inherited = task->inherited;
}
...
void gr_restore_rbac_exec_state(struct gr_rbac_exec_state *state)
{
	...
	task->acl = state->acl;
	task->exec_file = state->exec_file;
	task->is_writable = state->is_writable;
	task->inherited = state->inherited;
}
